...
#
# Each program that is under test shall write entrie in a common logfiile:
#
# testdir/etc/logfile
#
...
####################
mdtestnumberfile=${iortestdir}/${upper_exec}.TESTNUMBER
...
####################
rm -f ${iortestdir}/{lock,lock_process_${USER}_$$}
...
then
	...
	errecho ${LINENO} "Could not find srun on this machine"
	errecho ${LINENO} "If you need to run mpirun.  fix the code here"
	exit 1
fi
...
#	errecho ${LINENO} "MaxNodes=${MaxNodes}"
	...
	####################
	mdtestlog=${iortestdir}/${upper_exec}.testlog.txt
...
#	errecho ${LINENO} ${FUNCNAME} "IOR_PROCRATE=${IOR_PROCRATE}"
	...
	####################
	# if it came back zero, there is an invalid value in the datbase for
	...
	then
		...
		if [ "${procrate_value}" -eq 0 ]
		then
			errecho ${LINENO} "Invalid lookup from get_procrate, procrate_value=${procrate_value}"
			exit 1
		fi
	fi
